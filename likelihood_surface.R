...
fr <- fr %>% mutate(lik= map2_dbl(.$depth,.$alt, model_likelihood),
...
p2 <- ggplot(fr,aes(x=depth,y=alt)) + geom_contour(aes(z=vaf,colour=stat(level)),binwidth=0.005)  + scale_y_continuous(breaks=seq(1,10,1))#+ geom_contour(aes(z=tlod,colour=stat(level)))
...
p1 <- ggplot(fr,aes(x=depth,y=alt,z=tlod)) + geom_contour(aes(colour=stat(level)),binwidth=1) + scale_y_continuous(breaks=seq(1,10,1))
p4 <- direct.label(p1,list("far.from.others.borders", "calc.boxes", "enlarge.box", hjust=1,vjust=1,
                           box.color = NA, fill = "transparent", "draw.rects")) + 
  theme_bw() + ggtitle("TLOD contours")

p5 <- ggplot(fr,aes(x=depth,y=alt,z=log_expected_errors)) + geom_contour(aes(colour=stat(level))) + scale_y_continuous(breaks=seq(1,10,1))
...
p7 <- ggplot(fr,aes(x=depth,y=alt,z=log_expected_true)) + geom_contour(aes(colour=stat(level)),binwidth=1) + scale_y_continuous(breaks=seq(1,10,1))
...
p_final <- cowplot::plot_grid(p3,p4,p6,p8,nrow=2)
